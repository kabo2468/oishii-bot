...
import type { User } from '../misskey/api.js';
...
export default class extends Module {
  Name = 'Learn';
  Regex = new RegExp(
    `(.+?)[はも]?(${variables.food.good}|${variables.food.bad})よ?`,
  );
  LogName = 'LERN';
  ...
  async Run(bot: Bot, note: Note): Promise<void> {
    ...
    const user = await bot.api
      .call<User>('users/show', { userId: note.note.userId })
      .then((res) => res.body);

    const roles = user.roles || [];
    if (
      bot.config.denyRoleIds.some((id) => roles.some((role) => role.id === id))
    ) {
