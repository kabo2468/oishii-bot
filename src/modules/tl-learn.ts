import {
  extract as extractMfmNodes,
  toString as mfmNodesToString,
  parse as parseMfm,
} from 'mfm-js';
...
export default class extends Module {
  ...
  async Run(bot: Bot, note: Note): Promise<void> {
    ...
    const noMfmNodes = extractMfmNodes(mfmNodes, (n) =>
      ['text', 'unicodeEmoji', 'emojiCode'].includes(n.type),
    );
    const textNodes = extractMfmNodes(
      noMfmNodes,
      (node) => node.type === 'text',
    );
    ...
    const textArr = textNodes.map((n) =>
      n.type === 'text' ? n.props.text.trim() : '',
    );
    ...
    const foods = [
      ...(await getNouns(textStr, bot.config.mecab)),
      ...emojisArr,
    ];
    if (foods.length < 1) {
