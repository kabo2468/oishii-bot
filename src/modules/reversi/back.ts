...
import type { UserLite } from '../../misskey/api.js';
import type { CreatedNote } from '../../misskey/note.js';
import { chooseOneFromArr } from '../../utils/cofa.js';
import { botVersion } from '../../utils/version.js';
import { type Color, Game } from './engine.js';
import type { ReversiMatch } from './reversi.js';
...
class Back {
  ...
  async onInit(body: InitMes['body']) {
    ...
    this._inviter =
      this._game.user1Id === this._config.userId
        ? this._game.user2
        : this._game.user1;
  ...
  async onStarted(body: StartedMes['body']) {
    ...
    const text = messages.games.reversi.started(
      this.userName,
      `${this._config.host}/reversi/g/${this._game.id}`,
    );
    ...
    this._maxTurn =
      this._engine.map.filter((p) => p === 'empty').length -
      this._engine.board.filter((x) => x != null).length;
    ...
    this._engine.map.forEach((pix, i) => {
      ...
      const get = (x: number, y: number) => {
        if (
          x < 0 ||
          y < 0 ||
          x >= this._engine.mapWidth ||
          y >= this._engine.mapHeight
        )
          return 'null';
  ...
  think() {
    ...
    const canPutCorner = this._corners.filter((pos) =>
      this._engine.canPut(this._botColor, pos),
    );
  ...
  async post({
    ...
    return got
      .post<{ createdNote: CreatedNote }>(
        `${this._config.apiUrl}/notes/create`,
        {
          json: data,
          headers: {
