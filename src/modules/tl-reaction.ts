...
export default class extends Module {
  ...
  async Run(bot: Bot, note: Note): Promise<void> {
    // Local === null
    if (
      note.note.user.host !== null &&
      note.note.user.instance?.softwareName !== 'misskey'
    ) {
    ...
    const textAndEmojiNodes = extractMfmNodes(
      nodes,
      (n) => n.type === 'text' || n.type === 'unicodeEmoji',
    );
    ...
    const foundFood = foods.filter((food) =>
      food.keywords.some((keyword) => nouns.includes(keyword)),
    );
